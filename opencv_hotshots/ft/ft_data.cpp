        ...
        const vector<int> &pts)
{
  ...
  if(pts.size() == 0){
    for(int i = 0; i < n; i++){
      if(!flipped)circle(im,points[idx][i],1,color,2,LINE_AA);
      else{
    Point2f p(im.cols - 1 - points[idx][symmetry[i]].x,
          points[idx][symmetry[i]].y);
    circle(im,p,1,color,2,LINE_AA);
      }
    }
  }else{
    int m = pts.size();
     ...
     const vector<int> &pts)
{
  ...
  if(pts.size() == 0){
    for(int i = 0; i < n; i++){circle(im,p[i],1,colors[i],2,LINE_AA);}
  }else{
    int m = pts.size();
    for(int j = 0; j < m; j++){
      int i = pts[j]; if((i < 0) || (i >= n))continue;
      circle(im,p[i],1,colors[i],2,LINE_AA);
    }
  }
}
//==============================================================================
