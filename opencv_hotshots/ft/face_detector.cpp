      ...
      const Size minSize)
{
  ...
  for(int i = 0; i < data.n_images(); i++){
    ...
    if(faces.size() >= 1){
      if(visi){
    Mat I; cvtColor(im,I,COLOR_GRAY2RGB);
    for(int i = 0; i < n; i++)circle(I,p[i],1,CV_RGB(0,255,0),2,LINE_AA);
  ...
  cv::sort(X,Xsort,SORT_EVERY_COLUMN|SORT_ASCENDING); int nx = Xsort.rows;
  cv::sort(Y,Ysort,SORT_EVERY_COLUMN|SORT_ASCENDING); int ny = Ysort.rows;
  cv::sort(Z,Zsort,SORT_EVERY_COLUMN|SORT_ASCENDING); int nz = Zsort.rows;
