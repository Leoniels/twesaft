...
public:
  ...
  void
  draw_chosen_point(){
    if(pidx >= 0)circle(image,data.points[idx][pidx],1,CV_RGB(0,255,0),2,LINE_AA);
  }
  void
  draw_connections(){
    ...
    circle(image,data.points[idx][i],1,CV_RGB(0,255,0),2,LINE_AA);
  ...
  draw_symmetry(){
    ...
    for(int i = 0; i < int(data.symmetry.size()); i++){
      ...
      if(j != i){
    circle(image,data.points[idx][i],1,CV_RGB(255,255,0),2,LINE_AA);
    circle(image,data.points[idx][j],1,CV_RGB(255,255,0),2,LINE_AA);
      }
    }
    if(pidx >= 0)circle(image,data.points[idx][pidx],1,CV_RGB(0,255,0),2,LINE_AA);
  }
